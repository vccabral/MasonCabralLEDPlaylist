...
void (* g[1])() = { animationAColorSwipeRed };
void (* h[1])() = { animationBAudio };
int times[1] = {16};
int blends[1] = {8};
...
int master_play_list_index = 176;
int master_play_list_size = 192;
byte blend_amount[] = {
...
void loop() {
  ...
  if(master_play_list_index < times[0]){
    f[master_play_list_index/16]();                                               // render the first animation into leds2   
    g[master_play_list_index/16]();                                               // render the first animation into leds2   
  }
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[i] = blend( ledsA[i], ledsB[i], blends[0] );
  }
  FastLED.show();
